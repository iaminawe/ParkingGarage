...
import { MockGarageAPI } from '../helpers/testUtils';
import {
  CheckInResponse,
  CheckOutResponse,
  SpotsResponse,
...
describe('Performance and Load Tests', () => {
  ...
  describe('Load Testing', () => {
    test('should handle 100 vehicles per hour (normal load)', async () => {
      ...
      const checkIns: LoadTestVehicle[] = [];
      const checkOuts: CheckOutResult[] = [];
    ...
    test('should maintain consistency under stress', async () => {
      const stressOperations: Promise<request.Response>[] = [];
      ...
      const { total, available, occupied }: GarageSummary = spots.summary!;
  ...
  describe('Query Performance', () => {
    test('should efficiently query large result sets', async () => {
      ...
      for (const query of queries) {
        ...
        expect(spots).toHaveProperty('spots');
      }
    });
    ...
    test('should handle rapid spot status checks', async () => {
      ...
      const statusChecks: Promise<request.Response>[] = [];
  ...
  describe('Memory and Resource Management', () => {
    ...
    test('should handle maximum capacity efficiently', async () => {
      ...
      const error: APIError = overflow.body;
  ...
  describe('Recovery and Resilience', () => {
    test('should recover from rapid state changes', async () => {
      ...
      for (let cycle = 0; cycle < 10; cycle++) {
        ...
        const checkInPromises: Promise<request.Response>[] = [];
        for (let i = 0; i < 10; i++) {
    ...
    test('should handle malformed requests gracefully under load', async () => {
      const malformedRequests: Promise<request.Response>[] = [];
