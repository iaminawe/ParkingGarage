 ...
 */
router.get('/', authenticate, paymentOperationsLimiter, (async (
  req: AuthRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    if (vehicleId) {
      criteria.vehicleId = vehicleId as string;
    }
    if (sessionId) {
      criteria.sessionId = sessionId as string;
    }
    if (status) {
      criteria.status = status as PaymentStatus;
    }
    if (type) {
      criteria.type = type as PaymentType;
    }
    if (method) {
      criteria.method = method as PaymentMethod;
    }
    if (startDate) {
      criteria.startDate = new Date(startDate as string);
    }
    if (endDate) {
      criteria.endDate = new Date(endDate as string);
    }
    if (minAmount) {
      criteria.minAmount = parseFloat(minAmount as string);
    }
    if (maxAmount) {
      criteria.maxAmount = parseFloat(maxAmount as string);
    }
    if (licensePlate) {
      criteria.licensePlate = licensePlate as string;
    }
    if (transactionId) {
      criteria.transactionId = transactionId as string;
    }
    if (paymentNumber) {
      criteria.paymentNumber = paymentNumber as string;
    }
    ...
    const options = {
      ...
      orderBy: {
        [sortBy as string]: (sortOrder as string) === 'desc' ? ('desc' as const) : ('asc' as const),
      },
    };
 ...
 */
router.get('/:id', authenticate, managerOrAdmin, paymentOperationsLimiter, (async (
  req: AuthRequest,
  res: Response
): Promise<void> => {
  try {
 ...
 * @access  Private (Staff only)
 */
router.post('/', authenticate, managerOrAdmin, sensitivePaymentLimiter, (async (
  req: AuthRequest,
  res: Response
): Promise<void> => {
  try {
 ...
 * @access  Private (Staff only)
 */
router.put('/:id/process', authenticate, managerOrAdmin, sensitivePaymentLimiter, (async (
  req: AuthRequest,
  res: Response
): Promise<void> => {
  try {
 ...
 */
router.get('/session/:sessionId', authenticate, paymentOperationsLimiter, (async (
  req: AuthRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    const canAccess =
      ...
      payments.some(payment => payment.vehicleId === currentUser.id);
 ...
 * @access  Private (Staff only)
 */
router.put('/:id/fail', authenticate, managerOrAdmin, sensitivePaymentLimiter, (async (
  req: AuthRequest,
  res: Response
): Promise<void> => {
  try {
