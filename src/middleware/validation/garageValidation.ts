 ...
 */
export function validateGarageInitialization(req: Request, res: Response, next: NextFunction): void {
  const { body } = req;
  const errors: string[] = [];
  ...
  if (Array.isArray(body.floors)) {
    body.floors.forEach((floor: any, index: number) => {
    ...
    const floorNumbers = body.floors.map((f: any) => f.number);
    const duplicates = floorNumbers.filter((num: number, idx: number) => floorNumbers.indexOf(num) !== idx);
    if (duplicates.length > 0) {
      errors.push(`Duplicate floor numbers found: ${[...new Set(duplicates)].join(', ')}`);
    }

    // Validate total spots don't exceed reasonable limits
    const totalSpots = body.floors.reduce((sum: number, floor: any) => sum + (floor.bays * floor.spotsPerBay), 0);
    if (totalSpots > 10000) {
 ...
 */
export function validateRateUpdate(req: Request, res: Response, next: NextFunction): void {
  const { body } = req;
  const errors: string[] = [];

  // Check if body has at least one rate field
  const validRateTypes: (keyof RateStructure)[] = ['standard', 'compact', 'oversized', 'ev_charging'];
  const providedRates = Object.keys(body).filter(key => validRateTypes.includes(key as keyof RateStructure));
  ...
  validRateTypes.forEach(rateType => {
    if (body.hasOwnProperty(rateType)) {
      ...
      if (typeof rate !== 'number' || rate < 0 || rate > 1000) {
        errors.push(`${String(rateType)} rate must be a number between 0 and 1000`);
      }
    }
  });

  // Check for invalid rate types
  const invalidRates = Object.keys(body).filter(key => !validRateTypes.includes(key as keyof RateStructure));
 ...
 */
export function validateGarageQuery(req: Request, res: Response, next: NextFunction): void {
  const { includeStats, includeSpots } = req.query;
  const errors: string[] = [];

  // Validate boolean parameters
  if (includeStats !== undefined && !['true', 'false'].includes(includeStats as string)) {
    errors.push('includeStats parameter must be "true" or "false"');
  }

  if (includeSpots !== undefined && !['true', 'false'].includes(includeSpots as string)) {
  ...
  // Convert string booleans to actual booleans and store in req.query
  (req.query as any).includeStats = (includeStats as string) === 'true';
  (req.query as any).includeSpots = (includeSpots as string) === 'true';
 ...
 */
export function validateGarageConfigUpdate(req: Request, res: Response, next: NextFunction): void {
  const { body } = req;
  const errors: string[] = [];
 ...
 */
export function requireGarageExists(req: Request, res: Response, next: NextFunction): void {
 ...
 */
export function sanitizeGarageName(req: Request, res: Response, next: NextFunction): void {
