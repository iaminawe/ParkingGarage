 ...
 */
export const validateSpotUpdate = (
  req: TypedRequest<SpotUpdateBody>,
  res: Response,
  next: NextFunction
): void => {
  ...
  if (features !== undefined) {
    if (!Array.isArray(features)) {
      ...
      if (invalidFeatures.length > 0) {
        errors.push(
          `Invalid features: ${invalidFeatures.join(', ')}. Valid features: ${validFeatures.join(', ')}`
        );
      }
 ...
 */
export const sanitizeSpotUpdate = (
  req: TypedRequest<SpotUpdateBody>,
  res: Response,
  next: NextFunction
): void => {
 ...
 */
export const validateIncludeParams = (
  req: TypedRequest<any, SpotQueryParams>,
  res: Response,
  next: NextFunction
): void => {
