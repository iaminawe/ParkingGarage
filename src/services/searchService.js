...
class SearchService {
   ...
   * @returns {Object|null} Vehicle with location info or null if not found
   */
   ...
   * @param {string} searchTerm - Partial license plate to search for
   * @param {Object} options - Search options
   * @param {number} options.maxResults - Maximum number of results
   * @param {number} options.threshold - Fuzzy match threshold
   * @param {string} options.mode - Search mode ('partial', 'fuzzy', 'all')
   * @returns {Object} Search results with matches array
   */
  async searchVehicles(searchTerm, options = {}) {
    try {
      const {
        maxResults = 20,
        threshold = 0.6,
   ...
   * @param {number} location.floor - Floor number (optional)
   * @param {number} location.bay - Bay number (optional) 
   * @param {string} location.spotId - Specific spot ID (optional)
   * @returns {Array} Array of vehicles matching location criteria
   */
  async findVehiclesByLocation(location = {}) {
    try {
      ...
      throw new Error(`Failed to find vehicles by location: ${error.message}`);
    }
  }
   ...
   * @param {number} criteria.floor - Floor number (optional)
   * @param {number} criteria.bay - Bay number (optional)
   * @param {string} criteria.type - Spot type (optional)
   * @param {string[]} criteria.features - Required features (optional)
   * @returns {Array} Array of available spots matching criteria
   */
  async findAvailableSpots(criteria = {}) {
    try {
      ...
      // Apply filters
      if (floor) {
        availableSpots = availableSpots.filter(spot => spot.floor === parseInt(floor));
      }

      if (bay) {
        availableSpots = availableSpots.filter(spot => spot.bay === parseInt(bay));
      }
   ...
   * @param {string} partial - Partial license plate input
   * @param {number} limit - Maximum suggestions to return
   * @returns {Array} Array of license plate suggestions
   */
   ...
   * @private
   * @returns {string[]} Array of cached license plates
   */
  _getCachedLicensePlates() {
    const now = Date.now();
   ...
   */
  _calculateCurrentDuration(checkInTime) {
