...
export interface OccupancyReport {
  ...
  bySpotType: Record<VehicleType, {
    total: number;
    occupied: number;
    rate: number;
  }>;
  byFloor: Record<number, {
    total: number;
    occupied: number;
    rate: number;
  }>;
...
class ReportingService {
    ...
    try {
      ...
      const [totalSpots, currentOccupied] = await Promise.all([
        prisma.parkingSpot.count({
          where: { 
            isActive: true,
            ...(options.spotTypes && { spotType: { in: options.spotTypes.map(t => t.toUpperCase()) as any } }),
      ...
      const averageOccupancy = trends.length > 0 
  ...
  async generateRevenueReport(options: ReportOptions): Promise<RevenueReport> {
    try {
      ...
      Object.keys(byPaymentMethod).forEach(method => {
        byPaymentMethod[method].percentage = totalRevenue > 0 
          ? (byPaymentMethod[method].amount / totalRevenue) * 100 
          : 0;
      });

      // Revenue by spot type
      const bySpotType: Record<VehicleType, { revenue: number; sessions: number; averageValue: number }> = {
  ...
  async generateUserActivityReport(options: ReportOptions): Promise<UserActivityReport> {
    try {
      ...
      const userActivity = new Map<string, { sessionCount: number; totalSpent: number; email: string }>();
  ...
  private generateOccupancyTrends(
    ...
    for (let i = 23; i >= 0; i--) {
      ...
      const occupiedSpots = Math.floor(Math.random() * totalSpots * 0.8) + Math.floor(totalSpots * 0.1);
