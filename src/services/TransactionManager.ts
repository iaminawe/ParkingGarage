...
export class TransactionManager implements ITransactionManager {
   ...
   */
  private initializeTransactionMetrics(
    context: ITransactionContext,
    startTime: Date
  ): void {
  ...
  private updateTransactionMetrics(context: ITransactionContext): void {
    ...
    if (metrics) {
      ...
      metrics.duration = context.endTime ? 
        context.endTime.getTime() - metrics.startTime.getTime() : undefined;
  ...
  getTransactionStatistics(): {
    ...
    const averageDuration = completedMetrics.length > 0 ?
      completedMetrics.reduce((sum, m) => sum + (m.duration || 0), 0) / completedMetrics.length : 0;
